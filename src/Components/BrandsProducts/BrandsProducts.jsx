...
export default function BrandsProducts() {
    let { id } = useParams();
    function brandsProduct(x) {
        return axios.get(`https://ecommerce.routemisr.com/api/v1/brands/${x}`)
    }
    let { data, isLoading, isRefetching } = useQuery("brandsProduct", () => brandsProduct(id))
        ...
        <>
            {isRefetching ? <div className='w-100 py-5 d-flex justify-content-center margT'>
                ...
                <div className='text-center '>
                    ...
                    <img className='my-2 rounded' height={300} src={item?.image} alt="" />
                </div>
            </div>
            }
        </>
